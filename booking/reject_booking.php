...
if (isset($_GET['booking_id'])) {
    ...
    $updateQuery = "UPDATE booking SET status = 2 WHERE booking_Id = '$bookingId'";
    if (mysqli_query($conn, $updateQuery)) {
        // Redirect back to the dashboard with a success message
        header('Location: ../tutors/tutor-dashboard.php?section=2&message=Booking rejected successfully');
    } else {
        // Redirect back to the dashboard with an error message
        header('Location: ../tutors/tutor-dashboard.php?section=2&message=Failed to reject booking');
    }
} else {
